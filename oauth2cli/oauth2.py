...
class Client(BaseClient):  # We choose to implement all 4 grants in 1 class
    ...
    def obtain_token_by_device_flow(self, flow, **kwargs):
        # type: (dict, **dict) -> dict
        """Obtain token by a device flow object.

        Args:
            flow (dict): An object previously generated by initiate_device_flow(...).
            Note that this method will also UPDATE such object during each run.

        Conceptually, this device flow protocol has a polling nature. However,
        as a design decision, this method is NOT implemented as a blocking function,
        otherwise we would have to have another API for you to cancel a polling.
        Instead, we provide this non-blocking method only, and the caller is
        expected to call this method repeatedly, with proper interval. Such as:

            flow = client.initiate_device_flow(scope=...)
            ...
            while your_condition is True:
                result = client.obtain_token_by_device_flow(flow)
                if result.get("error") not in client.DEVICE_FLOW_RETRIABLE_ERRORS:
                    break
