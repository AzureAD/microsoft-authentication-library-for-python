...
class TestCertificateCache(unittest.TestCase):
    ...
    def test_insufficient_lifetime_not_cached(self):
        # Not enough remaining lifetime (< MIN_REMAINING_LIFETIME_SEC)
        from msal.msi_v2 import _CertCacheEntry
        half_threshold = _CertCacheEntry.MIN_REMAINING_LIFETIME_SEC // 2
        entry = self._make_entry(not_after=time.time() + half_threshold)
