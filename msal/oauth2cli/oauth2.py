...
class BaseClient(object):
    ...
    def _obtain_token(  # The verb "obtain" is influenced by OAUTH2 RFC 6749
        ...
        request_kwargs = self.get_obtain_token_request_info(grant_type=grant_type, data=data)
        request_kwargs.update(dict(params=params, timeout=timeout or self.timeout))
        request_kwargs.update(**kwargs)
        resp = (post or self.session.post)(**request_kwargs)
        return self.validate_obtain_token_response(resp)
    ...
    def requires_assertion(self):
        return self.default_body.get("client_assertion_type") and self.client_assertion

    def requires_auth(self):
        return self.client_secret and self.client_id

    def validate_obtain_token_response(self, resp):
    ...
    def get_obtain_token_request_info(self, grant_type, data=None, headers=None):
        validate_token_endpoint(self.configuration)
        data = self.build_obtain_token_request_data(grant_type=grant_type, data=data)
        ...
        auth = (self.client_id, self.client_secret) if self.requires_auth() else None  # for HTTP Basic Auth

        _headers = {'Accept': 'application/json'}
        _headers.update(headers or {})

        return {"url": self.configuration["token_endpoint"], "data": data, "auth": auth, "headers": _headers}

    def build_obtain_token_request_data(self, grant_type, data=None):
...
class Client(BaseClient):  # We choose to implement all 4 grants in 1 class
    ...
    def get_initiate_device_flows_request_info(self, scope):
        return dict(url=self.configuration[DAE], data={"client_id": self.client_id,
                                                       "scope": self._stringify(scope or [])})

    def build_obtain_token_by_device_flow_data(self, data, flow):
        data.update({
    ...
    def build_obtain_token_by_authorization_code_data(self, data, code, redirect_uri, scope):
    ...
    def build_obtain_token_by_username_password_data(self, data, username, password, scope):
        data.update(username=username, password=password, scope=scope)
        return data

    def build_obtain_token_for_client_data(self, data, scope):
        data.update(scope=scope)
        return data

    def build_obtain_token_for_assertion_data(self, data, scope, assertion, grant_type):
        encoder = self.grant_assertion_encoders.get(grant_type, lambda a: a)
        data.update(scope=scope, assertion=encoder(assertion))
        return data

    def handle_valid_token_response(self, resp, _data, grant_type, params):
        _resp = resp.copy()
    ...
    def initiate_device_flow(self, scope=None, timeout=None, **kwargs):
        ...
        """
        validate_device_authorization_endpoint(self.configuration)
        request_kwargs = self.get_initiate_device_flows_request_info(scope=scope)
        request_kwargs.update(kwargs)
    ...
    def _obtain_token_by_device_flow(self, flow, **kwargs):
        ...
        data = self.build_obtain_token_by_device_flow_data(data=kwargs.pop("data", {}), flow=flow)
        result = self._obtain_token(self.DEVICE_FLOW["GRANT_TYPE"], data=data, **kwargs)
    ...
    def build_auth_request_uri(
        ...
        """
        validate_authorization_endpoint(self.configuration)
    ...
    def obtain_token_by_authorization_code(
        ...
        """
        data = self.build_obtain_token_by_authorization_code_data(data=kwargs.pop("data", {}),
    ...
    def obtain_token_by_username_password(
        ...
        data = self.build_obtain_token_by_username_password_data(data=kwargs.pop("data", {}), username=username, password=password,
                                                                 scope=scope)
    ...
    def obtain_token_for_client(self, scope=None, **kwargs):
        ...
        """
        data = self.build_obtain_token_for_client_data(scope=scope)
    ...
    def _obtain_token(self, grant_type, params=None, data=None, *args, **kwargs):
        ...
        resp = super(Client, self)._obtain_token(grant_type, params, _data, *args, **kwargs)
        if "error" not in resp:
            self.handle_valid_token_response(resp=resp, _data=_data, grant_type=grant_type, params=params)
