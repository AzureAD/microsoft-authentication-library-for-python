...
This module implements the MSI v2 token acquisition path using Windows native APIs via ctypes:
...
Environment variables (optional):
  ...
  - MSAL_MSI_V2_KEY_NAME: override the per-boot key name (otherwise derived from metadata clientId)
  - MSAL_MSI_V2_ATTESTATION_CACHE: "0" to disable MAA JWT caching (implemented in msi_v2_attestation.py)
"""
...
# Legacy KeySpec values used with NCryptCreatePersistedKey.dwLegacyKeySpec and
# CRYPT_KEY_PROV_INFO.dwKeySpec when dwProvType==0 (CNG/KSP).
_AT_KEYEXCHANGE = 1
_AT_SIGNATURE = 2

# Flags used by CRYPT_KEY_PROV_INFO.dwFlags for CNG keys (best-effort: no UI prompts)
...
def _check_security_status(status: int, what: str) -> None:
    """Check SECURITY_STATUS return codes from NCrypt (0 == success)."""
    from .managed_identity import MsiV2Error
    if int(status) != 0:
...
def _der_context_implicit_constructed(tagnum: int, inner_content: bytes) -> bytes:
    """Context-specific IMPLICIT, constructed (CSR attributes use [0] IMPLICIT)."""
    return _der(0xA0 + tagnum, inner_content)


def _der_name_cn_dc(cn: str, dc: str) -> bytes:
    """Encode X.500 Name with CN and DC RDNs (CN UTF8String, DC IA5String if ASCII)."""
...
def _der_algid_rsapss_sha256() -> bytes:
    """AlgorithmIdentifier for RSASSA-PSS with SHA-256, MGF1(SHA-256), saltLength=32."""
...
def _open_or_create_keyguard_rsa_key(win32: Dict[str, Any], *, key_name: str) -> Tuple[Any, Any, str, bool]:
    ...
    # 2) Create if missing
    flags = (
        win32["NCRYPT_OVERWRITE_KEY_FLAG"]
    ...
    # Set properties only when we created the key.
...
def _build_csr_b64(win32: Dict[str, Any], key: Any, client_id: str, tenant_id: str, cu_id: Any) -> str:
    """
    Build CSR signed by KeyGuard key (RSA-PSS SHA256), including cuId request attribute.
    """
    ...
    # Attribute: SEQUENCE { OID, SET { <DER UTF8String(JSON)> } }
...
def _create_cert_context_with_key(
    ...
    *,
    ...
    """
    Create a CERT_CONTEXT from DER bytes and associate it with a CNG private key.

    We set multiple properties to maximize compatibility across Schannel consumers:
      - CERT_NCRYPT_KEY_HANDLE_PROP_ID (78): direct handle
      - CERT_KEY_CONTEXT_PROP_ID (5): CERT_KEY_CONTEXT union (best-effort)
      - CERT_KEY_PROV_INFO_PROP_ID (2): CRYPT_KEY_PROV_INFO so Schannel can reopen by name

    NOTE: For CNG keys (dwProvType==0), CRYPT_KEY_PROV_INFO.dwKeySpec must be AT_SIGNATURE or AT_KEYEXCHANGE.
    """
    ctypes_mod = win32["ctypes"]
    wintypes = win32["wintypes"]
    ...
    try:
        ...
        # (C) CERT_KEY_PROV_INFO_PROP_ID (so Schannel can reopen by name)
        ...
        prov_info = CRYPT_KEY_PROV_INFO(
            ...
            wintypes.DWORD(0),                  # dwProvType=0 for CNG/KSP
            wintypes.DWORD(_NCRYPT_SILENT_FLAG),
            wintypes.DWORD(0),
            None,
            wintypes.DWORD(_AT_SIGNATURE),      # IMPORTANT
        )
...
def _winhttp_post(win32: Dict[str, Any], url: str, cert_ctx: Any, body: bytes, headers: Dict[str, str]) -> Tuple[int, bytes]:
    """
    POST bytes to https URL using WinHTTP + SChannel, presenting the provided cert context.
    """
    from .managed_identity import MsiV2Error
    ...
    h_request = None
    try:
        # Best-effort: allow WinHTTP to send client cert for HTTP/2 negotiated connections.
...
def obtain_token(
    ...
    *,
    ...
    try:
        ...
        # 2) Open-or-create named per-boot KeyGuard RSA key
        ...
        att_jwt = get_attestation_jwt(
            ...
            key_handle=int(key.value),
            cache_key=f"{key_name}",  # stable per boot
        )
