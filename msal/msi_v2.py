...
def _load_win32() -> Dict[str, Any]:
    ...
    class BCRYPT_PSS_PADDING_INFO(ctypes.Structure):
        _fields_ = [("pszAlgId", ctypes.c_wchar_p), ("cbSalt", wintypes.ULONG)]
    ...
    ncrypt.NCryptOpenKey.argtypes = [NCRYPT_PROV_HANDLE, ctypes.POINTER(NCRYPT_KEY_HANDLE), ctypes.c_wchar_p, wintypes.DWORD, wintypes.DWORD]
    ncrypt.NCryptOpenKey.restype = SECURITY_STATUS
    ncrypt.NCryptCreatePersistedKey.argtypes = [NCRYPT_PROV_HANDLE, ctypes.POINTER(NCRYPT_KEY_HANDLE), ctypes.c_wchar_p, ctypes.c_wchar_p, wintypes.DWORD, wintypes.DWORD]
    ncrypt.NCryptCreatePersistedKey.restype = SECURITY_STATUS
    ncrypt.NCryptSetProperty.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p, ctypes.c_void_p, wintypes.DWORD, wintypes.DWORD]
    ...
    ncrypt.NCryptGetProperty.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p, ctypes.c_void_p, wintypes.DWORD, ctypes.POINTER(wintypes.DWORD), wintypes.DWORD]
    ncrypt.NCryptGetProperty.restype = SECURITY_STATUS
    ncrypt.NCryptExportKey.argtypes = [NCRYPT_KEY_HANDLE, NCRYPT_KEY_HANDLE, ctypes.c_wchar_p, ctypes.c_void_p, ctypes.c_void_p, wintypes.DWORD, ctypes.POINTER(wintypes.DWORD), wintypes.DWORD]
    ncrypt.NCryptExportKey.restype = SECURITY_STATUS
    ncrypt.NCryptSignHash.argtypes = [NCRYPT_KEY_HANDLE, ctypes.c_void_p, ctypes.c_void_p, wintypes.DWORD, ctypes.c_void_p, wintypes.DWORD, ctypes.POINTER(wintypes.DWORD), wintypes.DWORD]
    ...
    winhttp.WinHttpOpen.argtypes = [ctypes.c_wchar_p, wintypes.DWORD, ctypes.c_wchar_p, ctypes.c_wchar_p, wintypes.DWORD]
    ...
    winhttp.WinHttpOpenRequest.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_wchar_p, ctypes.c_void_p, wintypes.DWORD]
    ...
    winhttp.WinHttpSendRequest.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p, wintypes.DWORD, ctypes.c_void_p, wintypes.DWORD, wintypes.DWORD, ctypes.c_ulonglong]
    ...
    winhttp.WinHttpQueryHeaders.argtypes = [ctypes.c_void_p, wintypes.DWORD, ctypes.c_wchar_p, ctypes.c_void_p, ctypes.POINTER(wintypes.DWORD), ctypes.POINTER(wintypes.DWORD)]
    ...
    _WIN32 = {
        "ctypes": ctypes, "wintypes": wintypes, "ncrypt": ncrypt, "crypt32": crypt32, "winhttp": winhttp,
        "NCRYPT_PROV_HANDLE": NCRYPT_PROV_HANDLE, "NCRYPT_KEY_HANDLE": NCRYPT_KEY_HANDLE, "SECURITY_STATUS": SECURITY_STATUS,
        "CERT_CONTEXT": CERT_CONTEXT, "PCCERT_CONTEXT": PCCERT_CONTEXT, "BCRYPT_PSS_PADDING_INFO": BCRYPT_PSS_PADDING_INFO,
        "ERROR_SUCCESS": ERROR_SUCCESS, "NCRYPT_OVERWRITE_KEY_FLAG": NCRYPT_OVERWRITE_KEY_FLAG,
        "NCRYPT_LENGTH_PROPERTY": NCRYPT_LENGTH_PROPERTY, "NCRYPT_EXPORT_POLICY_PROPERTY": NCRYPT_EXPORT_POLICY_PROPERTY,
        "NCRYPT_KEY_USAGE_PROPERTY": NCRYPT_KEY_USAGE_PROPERTY, "NCRYPT_ALLOW_SIGNING_FLAG": NCRYPT_ALLOW_SIGNING_FLAG,
        "NCRYPT_ALLOW_DECRYPT_FLAG": NCRYPT_ALLOW_DECRYPT_FLAG, "BCRYPT_PAD_PSS": BCRYPT_PAD_PSS,
        "BCRYPT_SHA256_ALGORITHM": BCRYPT_SHA256_ALGORITHM, "BCRYPT_RSA_ALGORITHM": BCRYPT_RSA_ALGORITHM,
        "BCRYPT_RSAPUBLIC_BLOB": BCRYPT_RSAPUBLIC_BLOB, "BCRYPT_RSAPUBLIC_MAGIC": BCRYPT_RSAPUBLIC_MAGIC,
        "X509_ASN_ENCODING": X509_ASN_ENCODING, "PKCS_7_ASN_ENCODING": PKCS_7_ASN_ENCODING,
        "CERT_NCRYPT_KEY_HANDLE_PROP_ID": CERT_NCRYPT_KEY_HANDLE_PROP_ID, "CERT_SET_PROPERTY_INHIBIT_PERSIST_FLAG": CERT_SET_PROPERTY_INHIBIT_PERSIST_FLAG,
        "WINHTTP_ACCESS_TYPE_DEFAULT_PROXY": WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, "WINHTTP_FLAG_SECURE": WINHTTP_FLAG_SECURE,
        "WINHTTP_OPTION_CLIENT_CERT_CONTEXT": WINHTTP_OPTION_CLIENT_CERT_CONTEXT, "WINHTTP_OPTION_ENABLE_HTTP2_PLUS_CLIENT_CERT": WINHTTP_OPTION_ENABLE_HTTP2_PLUS_CLIENT_CERT,
        "WINHTTP_QUERY_STATUS_CODE": WINHTTP_QUERY_STATUS_CODE, "WINHTTP_QUERY_FLAG_NUMBER": WINHTTP_QUERY_FLAG_NUMBER,
    }
    return _WIN32
...
def _der_subject_public_key_info_rsa(modulus: int, exponent: int) -> bytes:
    ...
    alg = _der_sequence(_der_oid("1.2.840.113549.1.1.1"), _der_null())
...
def _ncrypt_get_property(win32: Dict[str, Any], h: Any, name: str) -> bytes:
    ...
    ncrypt = win32["ncrypt"]
    cb = wintypes.DWORD(0)
    status = ncrypt.NCryptGetProperty(h, name, None, 0, ctypes_mod.byref(cb), 0)
    if int(status) != 0 and cb.value == 0:
        _check_security_status(status, f"NCryptGetProperty({name})")
    if cb.value == 0:
        return b""
    buf = (ctypes_mod.c_ubyte * cb.value)()
    status = ncrypt.NCryptGetProperty(h, name, buf, cb.value, ctypes_mod.byref(cb), 0)
    _check_security_status(status, f"NCryptGetProperty({name})")
...
def _open_or_create_keyguard_rsa_key(win32: Dict[str, Any], *, key_name: str) -> Tuple[Any, Any, str, bool]:
    ctypes_mod = win32["ctypes"]
    wintypes = win32["wintypes"]
    ncrypt = win32["ncrypt"]
    prov = win32["NCRYPT_PROV_HANDLE"]()
    status = ncrypt.NCryptOpenStorageProvider(ctypes_mod.byref(prov), "Microsoft Software Key Storage Provider", 0)
    _check_security_status(status, "NCryptOpenStorageProvider")
    key = win32["NCRYPT_KEY_HANDLE"]()
    status = ncrypt.NCryptOpenKey(prov, ctypes_mod.byref(key), str(key_name), _AT_SIGNATURE, 0)
    if int(status) == 0:
    ...
    status = ncrypt.NCryptCreatePersistedKey(prov, ctypes_mod.byref(key), win32["BCRYPT_RSA_ALGORITHM"], str(key_name), _AT_SIGNATURE, flags)
    if _status_u32(status) == _NTE_EXISTS:
        status2 = ncrypt.NCryptOpenKey(prov, ctypes_mod.byref(key), str(key_name), _AT_SIGNATURE, 0)
        _check_security_status(status2, f"NCryptOpenKey({key_name}) after exists")
        return prov, key, str(key_name), True
    _check_security_status(status, "NCryptCreatePersistedKey")
    length = wintypes.DWORD(int(_RSA_KEY_SIZE))
    status = ncrypt.NCryptSetProperty(key, win32["NCRYPT_LENGTH_PROPERTY"], ctypes_mod.byref(length), ctypes_mod.sizeof(length), 0)
    ...
    status = ncrypt.NCryptSetProperty(key, win32["NCRYPT_KEY_USAGE_PROPERTY"], ctypes_mod.byref(usage), ctypes_mod.sizeof(usage), 0)
    _check_security_status(status, "NCryptSetProperty(Key Usage)")
    export_policy = wintypes.DWORD(0)
    status = ncrypt.NCryptSetProperty(key, win32["NCRYPT_EXPORT_POLICY_PROPERTY"], ctypes_mod.byref(export_policy), ctypes_mod.sizeof(export_policy), 0)
    ...
    if not vi or len(vi) < 4:
        from .managed_identity import MsiV2Error
        raise MsiV2Error("[msi_v2] Virtual Iso property not available; Credential Guard likely not active.")
...
def _ncrypt_sign_pss_sha256(win32: Dict[str, Any], key: Any, digest: bytes) -> bytes:
    ...
    status = ncrypt.NCryptSignHash(key, ctypes_mod.byref(pad), hash_buf, len(digest), None, 0, ctypes_mod.byref(cb_sig), win32["BCRYPT_PAD_PSS"])
    ...
    status = ncrypt.NCryptSignHash(key, ctypes_mod.byref(pad), hash_buf, len(digest), sig_buf, cb_sig.value, ctypes_mod.byref(cb_sig), win32["BCRYPT_PAD_PSS"])
...
def _create_cert_context_with_key(win32: Dict[str, Any], cert_der: bytes, key: Any, key_name: str, *, ksp_name: str = "Microsoft Software Key Storage Provider") -> Tuple[Any, Any, Tuple[Any, ...]]:
    ctypes_mod = win32["ctypes"]
    wintypes = win32["wintypes"]
    ...
    try:
        ...
        ok = crypt32.CertSetCertificateContextProperty(ctx, win32["CERT_NCRYPT_KEY_HANDLE_PROP_ID"], win32["CERT_SET_PROPERTY_INHIBIT_PERSIST_FLAG"], ctypes_mod.byref(key_handle))
        if not ok:
        ...
        class CERT_KEY_CONTEXT(ctypes_mod.Structure):
            _fields_ = [("cbSize", wintypes.DWORD), ("hCryptProvOrNCryptKey", ctypes_mod.c_void_p), ("dwKeySpec", wintypes.DWORD)]
        ...
        ok = crypt32.CertSetCertificateContextProperty(ctx, CERT_KEY_CONTEXT_PROP_ID, win32["CERT_SET_PROPERTY_INHIBIT_PERSIST_FLAG"], ctypes_mod.byref(key_ctx))
        if not ok:
        ...
        class CRYPT_KEY_PROV_INFO(ctypes_mod.Structure):
            _fields_ = [("pwszContainerName", wintypes.LPWSTR), ("pwszProvName", wintypes.LPWSTR), ("dwProvType", wintypes.DWORD), ("dwFlags", wintypes.DWORD), ("cProvParam", wintypes.DWORD), ("rgProvParam", ctypes_mod.c_void_p), ("dwKeySpec", wintypes.DWORD)]
        ...
        prov_info = CRYPT_KEY_PROV_INFO(ctypes_mod.cast(container_buf, wintypes.LPWSTR), ctypes_mod.cast(provider_buf, wintypes.LPWSTR), wintypes.DWORD(0), wintypes.DWORD(_NCRYPT_SILENT_FLAG), wintypes.DWORD(0), None, wintypes.DWORD(_AT_SIGNATURE))
        keepalive.append(prov_info)
        ok = crypt32.CertSetCertificateContextProperty(ctx, CERT_KEY_PROV_INFO_PROP_ID, win32["CERT_SET_PROPERTY_INHIBIT_PERSIST_FLAG"], ctypes_mod.byref(prov_info))
        if not ok:
...
def _winhttp_post(win32: Dict[str, Any], url: str, cert_ctx: Any, body: bytes, headers: Dict[str, str]) -> Tuple[int, bytes]:
    ...
    if u.query:
        path += "?" + u.query
    h_session = winhttp.WinHttpOpen("msal-python-msi-v2", win32["WINHTTP_ACCESS_TYPE_DEFAULT_PROXY"], None, None, 0)
    if not h_session:
    ...
    try:
        ...
        try:
            winhttp.WinHttpSetOption(h_session, win32["WINHTTP_OPTION_ENABLE_HTTP2_PLUS_CLIENT_CERT"], ctypes_mod.byref(enable), ctypes_mod.sizeof(enable))
        except Exception:
            pass
        h_connect = winhttp.WinHttpConnect(h_session, host, int(port), 0)
        if not h_connect:
            _raise_win32_last_error("[msi_v2] WinHttpConnect failed")
        h_request = winhttp.WinHttpOpenRequest(h_connect, "POST", path, None, None, None, win32["WINHTTP_FLAG_SECURE"])
        if not h_request:
            _raise_win32_last_error("[msi_v2] WinHttpOpenRequest failed")
        CertContext = win32["CERT_CONTEXT"]
        ok = winhttp.WinHttpSetOption(h_request, win32["WINHTTP_OPTION_CLIENT_CERT_CONTEXT"], cert_ctx, ctypes_mod.sizeof(CertContext))
        if not ok:
        ...
        ok = winhttp.WinHttpSendRequest(h_request, header_lines, 0xFFFFFFFF, body_buf, len(body), len(body), 0)
        if not ok:
        ...
        ok = winhttp.WinHttpQueryHeaders(h_request, win32["WINHTTP_QUERY_STATUS_CODE"] | win32["WINHTTP_QUERY_FLAG_NUMBER"], None, ctypes_mod.byref(status), ctypes_mod.byref(status_size), ctypes_mod.byref(index))
        if not ok:
...
def _acquire_token_mtls_schannel(win32: Dict[str, Any], token_endpoint: str, cert_ctx: Any, client_id: str, scope: str) -> Dict[str, Any]:
    from .managed_identity import MsiV2Error
    from urllib.parse import urlencode
    form = urlencode({"grant_type": "client_credentials", "client_id": client_id, "scope": scope, "token_type": "mtls_pop"}).encode("utf-8")
    status, resp_body = _winhttp_post(win32, token_endpoint, cert_ctx, form, headers={"Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"})
...
def obtain_token(http_client, managed_identity: Dict[str, Any], resource: str, *, attestation_enabled: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        key_name = os.getenv(_KEY_NAME_ENVVAR) or _stable_key_name(str(client_id))
        ...
        att_jwt = get_attestation_jwt(attestation_endpoint=str(attestation_endpoint), client_id=str(client_id), key_handle=int(key.value), cache_key=f"{key_name}")
        ...
        cred = _imds_post_json(http_client, issue_url, params, issue_headers, {"csr": csr_b64, "attestation_token": att_jwt})
